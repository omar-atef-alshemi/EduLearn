...
const generateTokens = (user) => {
  const accessToken = jwt.sign(
    { userInfo: { id: user._id, role: user.role, firstName: user.firstName, lastName: user.lastName } },
...
const refresh = (req, res) => {
  ...
  if (!cookies?.jwt) return res.status(401).json({ message: "Unauthorized: Refresh cookie missing" });
  ...
  jwt.verify(
    ...
    async (err, decoded) => {
      ...
      const userId = decoded.userInfo?.id || decoded.id;
      const user = await User.findById(userId).exec();
      
      if (!user) return res.status(401).json({ message: "Unauthorized: User not found during refresh" });
